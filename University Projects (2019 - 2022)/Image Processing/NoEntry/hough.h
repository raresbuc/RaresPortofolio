...
void hough_circles(Mat& input, Mat& gradient, Mat& dir, int*** hSpace, int radius, const int MAXRADIUS);
...
void hough_main(string image_nr) {
    Mat image;
    ...
    const int MAXRADIUS = min(image.rows / 5, image.cols / 5);
    ...
    // finding circles 
    hough_circles(image, TH, SobelDir, hSpace, MAXRADIUS - MINRADIUS, MAXRADIUS);
}
